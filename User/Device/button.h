...
#define LONG_PRESS_CNT 125  // 长按判断时间 指的是调用次数 如果8ms调一次 那就是1000ms之后算长按
...
/*
使用示例 当编号为1的按钮发生按键按下事件时将后右轮 wheel[0] 的目标速度反向
...
void server_button(BUTTON *button, BUTTON_STATION station) {
  if (button->which == 1) {                                                      // 判断按键编号 为1
    if (station == BUTTON_DOWN) {                                                // 判断按键事件 为BUTTON_DOWN
      status.motor.wheel[0].tar_speed = -status.motor.wheel[0].tar_speed;        // 反向后右轮目标速度
    }
  }
  return;
}
